"""
This script transforms raw inputs into relevant features, converts them into appropriate data types, drops rows/columns
with high missing rates as well as highly correlated or trivial columns. We also impute numerical columns with the best
approach found in `src/explore_imputation.py` script and create missing indicators or replace missing levels for
categorical variables. In the end, we select the best features from this newly engineered set based on K-S statistics.

Result: the final processed sets of train and test data with 185 features are used for modeling later on.
"""
